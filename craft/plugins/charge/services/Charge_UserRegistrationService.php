...
class Charge_UserRegistrationService extends BaseApplicationComponent
{
    ...
    public function postRegisterGuest(ChargeModel &$chargeModel)
    {
        ...
        $chargeModel->userId = $user->id;
        $chargeRecord = ChargeRecord::model()->findById($chargeModel->id);
        $chargeRecord->userId = $chargeModel->userId;
    ...
    private function registerGuestPending(ChargeModel $chargeModel)
    {
        ...
        if ($imageValidates && craft()->users->saveUser($user))
        {
            ...
            $this->account = $user;
            craft()->charge_log->success('New User account created in a suspended state', ['account' => $user]);
            return true;
        }
        else
        {
